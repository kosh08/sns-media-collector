# SNS Media Collector v0.4.0

X / Twitter と pixiv の画像・動画、Xブックマーク本文を、用途ごとの「取得設定」で収集するWindowsアプリです。

## 今回の更新

- 認証アカウントと取得設定を分離し、同じログインを複数用途で再利用できます。
- 取得設定はドラッグ＆ドロップで並び替えできます。
- Xブックマークを、画像のみ／本文＋画像／本文のみで保存できます。
- 固定方針で自動保存するか、確認箱で投稿ごとに振り分けるかを選べます。
- アプリ内Xログインから自分の安定IDを取得し、自分のURL・ユーザー名の二重入力をなくしました。
- v0.3系の作業設定は、初回起動時に取得設定へ自動移行します。
