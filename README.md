# SNS Media Collector v0.4.8

X / Twitter と pixiv の画像・動画、Xブックマーク本文を、用途ごとの「取得設定」で収集するWindowsアプリです。

## 今回の更新

- Xのいいね・ブックマークで「画像・動画のみ」「本文＋画像・動画」「本文のみ」の保存先を個別に設定できるようになりました。
- 確認箱で投稿ごとに保存方法を変えても、選んだ内容に対応する保存先へ自動で振り分けます。
- 本文＋画像・動画では、Markdownとメディアを同じ専用フォルダへまとめて保存します。
- 旧設定は従来のメディア保存先と本文保存先を保ったまま、新しい3保存先へ自動移行します。
