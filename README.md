# SNS Media Collector v0.4.3

X / Twitter と pixiv の画像・動画、Xブックマーク本文を、用途ごとの「取得設定」で収集するWindowsアプリです。

## 今回の更新

- 配布版の通常起動では検証用データフォルダ指定を無視し、`smc-smoke-*` が正規の保存場所になる問題を防止します。
- 検証用の一時保存先が画面へ残った場合、通常のLibraryへ自動修復します。
- 長いファイル名はカード内で中央を省略し、ツールチップで全文を確認できます。
- 最後に保存したファイルは、長いフルパスではなくファイル名を簡潔に表示します。
