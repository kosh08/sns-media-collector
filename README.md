# SNS Media Collector v0.4.9

X / Twitter と pixiv の画像・動画、Xブックマーク本文を、用途ごとの「取得設定」で収集するWindowsアプリです。

## 今回の更新

- 範囲変更・アンカー更新・取得完了後に、取得設定の保存先が既定Libraryへ戻る問題を修正しました。
- 取得設定の3保存先は、別の取得設定を選択したときだけ読み替え、通常の画面更新では上書きしません。
- 実行直前のLikes確認コマンドにも、画面で指定した保存先がそのまま渡ることを回帰テストで確認します。
- v0.4.8で追加した画像のみ／本文＋画像／本文のみの独立保存先と旧設定移行を維持します。
