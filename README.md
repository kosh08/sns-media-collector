# SNS Media Collector v0.4.1

X / Twitter と pixiv の画像・動画、Xブックマーク本文を、用途ごとの「取得設定」で収集するWindowsアプリです。

## 今回の更新

- 取得メニューを中心に画面を整理し、選択中の設定名と内容を中央上部にも表示します。
- ログイン管理は折りたたみ式になり、必要なときだけ展開できます。
- 選択色・見出し・余白・ボタンの優先順位を見直し、次の操作を見つけやすくしました。
- 処理がないときは右側に待機状態を表示し、開始後だけ進行状況へ切り替わります。
