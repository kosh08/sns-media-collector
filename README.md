# SNS Media Collector v0.4.6

X / Twitter と pixiv の画像・動画、Xブックマーク本文を、用途ごとの「取得設定」で収集するWindowsアプリです。

## 今回の更新

- X認証の推奨手順を「通常ブラウザでログイン→cookies.txt取り込み→認証テスト」に変更しました。
- cookies.txtはアカウント編集画面へのドラッグ＆ドロップでも取り込めます。
- Cookie内のXユーザーIDを自動で紐付け、別アカウントのCookieによる誤上書きを防止します。
- Xのアプリ内ログインは、X側に拒否される環境向けに「予備」として残します。
