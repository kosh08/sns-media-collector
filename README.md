# SNS Media Collector v0.4.10

X / Twitter と pixiv の画像・動画、Xブックマーク本文を、用途ごとの「取得設定」で収集するWindowsアプリです。

## 今回の更新

- Xの「すべて確認」で投稿本文が認識されず、アンカー未検出として止まる問題を修正しました。
- gallery-dlの正しい投稿イベントを使い、本文だけのいいねも確認箱へ取り込めるようにしました。
- 投稿メタデータを取得できない場合も、メディア情報から復旧して全件確認をアンカー判定へ誤接続しません。
- 同じ投稿イベントを使うXブックマークの本文取得もあわせて修正しました。
