# SNS Media Collector v0.4.4

X / Twitter と pixiv の画像・動画、Xブックマーク本文を、用途ごとの「取得設定」で収集するWindowsアプリです。

## 今回の更新

- `smc-smoke-*` に残った利用データを検出し、確認後に通常のデータフォルダへ復旧します。
- 元の保存先、取得設定、認証、Likes／ブックマークの差分位置、archive、保存済みファイルをまとめて引き継ぎます。
- 復旧前の現行設定をバックアップし、復旧元や既存の保存済みファイルは削除・上書きしません。
